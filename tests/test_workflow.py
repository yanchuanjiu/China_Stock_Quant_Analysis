...
class WorkflowTests(unittest.TestCase):
    def test_require_dependencies_success(self):
        fake_modules = {
            "qlib": SimpleNamespace(),
            "qlib.contrib.data.handler": SimpleNamespace(Alpha158="Alpha158"),
            "qlib.contrib.eval": SimpleNamespace(risk_analysis="risk"),
            "qlib.contrib.model.gbdt": SimpleNamespace(LGBModel="LGB"),
            "qlib.contrib.strategy.signal_strategy": SimpleNamespace(TopkDropoutStrategy="Strategy"),
            "qlib.data.dataset": SimpleNamespace(DatasetH="Dataset"),
            "qlib.data.dataset.handler": SimpleNamespace(DataHandlerLP="Handler"),
        ...
        with mock.patch.object(workflow.importlib, "import_module", side_effect=fake_import):
            deps = workflow.require_dependencies()
    ...
    def test_require_dependencies_missing_dependency(self):
        def boom(_):
            raise ImportError("missing")

        with mock.patch.object(workflow.importlib, "import_module", side_effect=boom):
            with self.assertRaises(RuntimeError):
                workflow.require_dependencies()

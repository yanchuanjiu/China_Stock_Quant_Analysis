...
## 2025-11-27 量比因子增强模型 + 账户余额跟踪

### 1. 问题与解决

| 问题 | 解决方案 |
|------|----------|
| 账户余额不准确 | 使用 Qlib 原生 `Account` 和 `Position` 模块，`verbose=True` 输出每笔交易详情 |
| 量比因子策略 | 基于 Alpha158 调整模型参数，突出量比因子重要性 |

### 2. Qlib 交易详情输出格式

每笔交易输出格式:
```
[I 2025-04-17 00:00:00]: buy SZ002459, price 0.92, amount 10010.91, deal_amount 10010.91, factor 0.036, value 9184.02, cash 14455.00
```
...
### 3. 量比因子 vs Alpha158 对比结果

| 模型 | 总收益率 | Sharpe | 最大回撤 | 最终市值 |
|------|----------|--------|----------|----------|
| **VolumeFactor_LightGBM** | **-2.90%** | **-1.40** | -5.79% | ¥97,098 |
| VolumeFactor_XGBoost | -3.80% | -1.72 | -7.52% | ¥96,198 |
| Alpha158_LightGBM | -6.88% | -3.47 | -7.79% | ¥93,117 |
| Alpha158_XGBoost | -6.18% | -3.97 | -8.03% | ¥93,822 |

**结论**: 量比因子增强模型在该测试期表现优于标准 Alpha158 模型

### 4. Alpha158 量比相关因子

```
VMA    - 成交量移动平均 (5/10/20/30/60日)
VSTD   - 成交量标准差
WVMA   - 成交量加权价格波动
CORR   - 价格与成交量相关性
CORD   - 价格变化与成交量变化相关性
...
```
日期         现金          持仓          总值         股票数
2025-04-16: ¥100,000.00  ¥0.00        ¥100,000.00  0
2025-04-17: ¥14,455.00   ¥84,820.07   ¥99,275.07   9
2025-04-18: ¥11,944.24   ¥86,989.97   ¥98,934.21   9
...
2025-05-14: ¥12,879.79   ¥84,218.16   ¥97,097.95   9
```

### 6. 报告位置

- **HTML 报告**: `output/volume_factor_backtest/volume_factor_report.html`
- **账户历史**: `output/volume_factor_backtest/*_account_history.csv`
- **指标对比**: `output/volume_factor_backtest/volume_factor_comparison.csv`

### 7. 新增脚本

`run_volume_factor_backtest.py` - 量比因子策略回测
